...
- In contrast to traditional scripts that typically support only a "single model × single benchmark" setup, ImageEvalKit enables evaluating **multiple models on multiple benchmarks** in a single run.
...
| `HF_HOME` | ❌ No | HuggingFace root cache directory; relevant models will be cached here. | `HF_HOME="/path/to/ImageEvalKit_dev/.cache/huggingface"` |
...
**Made with ❤️ by the ImageEvalKit Team**
